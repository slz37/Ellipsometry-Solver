...
def confidence_int(cov, alpha):
    #p-value
    per = abs(st.norm.ppf((1 - alpha) / 2))
    err = np.sqrt(np.diag(np.abs(cov))) * per
    
    return err
    

def print_results(fit_vars, phi, rho_act, cov, layers):
    ...
    for layer in layers:
        ...
        ci = confidence_int(cov, 0.9)
        
        #Print fit variables - always organized as [n, k, d]
        if layer.fit_n:
            print('\tn: ', fit_vars[0], '± {}'.format(ci[0]))
            
        if layer.fit_n and layer.fit_k:
            print('\tk: ', fit_vars[1], '± {}'.format(ci[1]))
        elif layer.fit_k:
            print('\tk: ', fit_vars[0], '± {}'.format(ci[0]))

        if layer.fit_n and layer.fit_k and layer.fit_d:
            print('\td: ', fit_vars[2], '± {}'.format(ci[2]))
        elif (layer.fit_n or layer.fit_k) and layer.fit_d:
            print('\td: ', fit_vars[1], '± {}'.format(ci[1]))
        elif layer.fit_d:
            print('\td: ', fit_vars[0], '± {}'.format(ci[0]))
    print('\n\tMSE: ', MSE)

def plot(phi, rho, fit_vars, cov, layers):
    ...
    print_results(fit_vars, phi, rho, cov, layers)
